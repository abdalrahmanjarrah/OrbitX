...
## 0.5 Error Review — CHECK ON EVERY SESSION START
- At the start of every session (before doing any other work), read `errors/latest.json` (generated by the server from `errors/errors.jsonl`) and report to the user any new errors: count, source, severity, how many times repeated. If it does not exist (no errors yet / server not run), say so in one line and continue.
- New browser errors are ingested by the app into `/api/errors/ingest` → written to `errors/errors.jsonl` + `errors/latest.json`. Do NOT modify these files manually.
- If the error file is getting large, you may summarize or rotate it only with the user's permission.
